...
-- Datatype :
	...
	-- Modify column datatype or column property.
		When we want to modify datatype of table and property of the table 
		...
		-- Example :
			...
			Alter column Student_FirstName nvarchar(15)

	-- Add new column :
		Add a new column into existing table without affecting existing data.
		...
		-- Example :
			Alter table tbl_Student
			Add Student_BloodGroup varchar(4)


	-- Remove column :
		Remove column from table without affecting existing data.
		...
		-- Example :
			Alter table tbl_Student
			Drop column Student_BloodGroup

	-- Rename column :
		Rename existing column without affect existing data.
		We need to change column name manually where it is referenced after 
			rename.

		-- Syntax :
			Exec sp_rename 'tbl_name.old_column_name','new_column_name','column'

		-- Example :
			exec sp_rename 'tbl_Student.Student_MobileNo','Student_Mobile','column'

	-- Rename table :
		Rename exsting table without affecting data.
		We need to change table name manually where it is referenced after 
			rename.

		-- Syntax :
			Exec sp_rename 'old_tbl_name','new_tbl_name'

		-- Example :
			Exec sp_rename 'tbl_Student','tbl_StudentMaster'

	-- Delete table :
		Delete existing table.
		When we delete any table with that data is also get deleted.
		Once we delete any table we do not have option to recover data or table.

		-- Syntax :
			Drop table tbl_name

		-- Example :
			Drop table tbl_StudentMaster